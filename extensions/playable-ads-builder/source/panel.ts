...
import { ICustomPanelThis, IOptions } from '../@types';
import { PACKAGE_NAME } from './global';
import { CHANNEL_SORTED, CHANNEL_FIELD_PREFIX, CompressionType } from './channels';

let panel: ICustomPanelThis;
// listener của từng chip kênh quảng cáo (danh sách kênh sinh động, không khai báo cứng trong $)
let channelChipBindings: Array<{ el: HTMLElement; handler: () => void }> = [];
 ...
 */
const PROFILE_TYPE: Editor.Profile.ProjectProtocol = 'project';
const PROFILE_KEYS: (keyof IOptions)[] = [
    'appleUrl', 'googleUrl', 'compressionType', 'compressionQuality', 'audioCompressionEnabled', 'audioBitrate',
];

async function loadSavedConfig(): Promise<Record<string, any>> {
    try {
        const saved = await Editor.Profile.getProject(PACKAGE_NAME, '', PROFILE_TYPE);
        return saved || {};
    } catch (err) {
        console.warn(`[${PACKAGE_NAME}] Không đọc được cấu hình đã lưu:`, err);
        return {};
    }
}

function saveConfig(key: string, value: any) {
    Editor.Profile.setProject(PACKAGE_NAME, key, value, PROFILE_TYPE).catch((err: any) => {
        console.warn(`[${PACKAGE_NAME}] Không lưu được cấu hình:`, err);
    });
}
...
function applySavedConfig(saved: Record<string, any>) {
    PROFILE_KEYS.forEach(key => {
        if (saved[key] !== undefined) {
            (panel.options as any)[key] = saved[key];
            panel.dispatch('update', `packages.${PACKAGE_NAME}.${key}`, saved[key]);
        }
    });
    CHANNEL_SORTED.forEach(channel => {
        const fieldKey = `${CHANNEL_FIELD_PREFIX}${channel}`;
        if (saved[fieldKey] !== undefined) {
            (panel.options as any)[fieldKey] = saved[fieldKey];
            panel.dispatch('update', `packages.${PACKAGE_NAME}.${fieldKey}`, saved[fieldKey]);
        }
    });
}
...
const channelRowsHtml = CHANNEL_SORTED.map(channel => `<div class="channel-chip" data-channel="${channel}">${channel}</div>`).join('');

export const template = `
<div class="playable-ads-panel">
    <ui-prop id="appleUrlProp">
        <ui-label slot="label" value="i18n:${PACKAGE_NAME}.options.appleUrl"></ui-label>
    ...
    <ui-prop id="googleUrlProp">
        <ui-label slot="label" value="i18n:${PACKAGE_NAME}.options.googleUrl"></ui-label>
        <div slot="content" class="link-row">
    ...
    <ui-prop id="compressionTypeProp">
        <ui-label slot="label" value="i18n:${PACKAGE_NAME}.options.compressionType"></ui-label>
        <ui-select slot="content">
            <option value="none">i18n:${PACKAGE_NAME}.options.compressionNone</option>
            <option value="lossless">i18n:${PACKAGE_NAME}.options.compressionLossless</option>
            <option value="lossy">i18n:${PACKAGE_NAME}.options.compressionLossy</option>
        </ui-select>
    </ui-prop>
    <ui-prop id="compressionQualityProp">
        <ui-label slot="label" value="i18n:${PACKAGE_NAME}.options.compressionQuality"></ui-label>
        <ui-num-input slot="content" step="1" min="0" max="100"></ui-num-input>
    </ui-prop>

    <ui-prop id="audioCompressionEnabledProp">
        <ui-label slot="label" value="i18n:${PACKAGE_NAME}.options.audioCompressionEnabled"></ui-label>
        <ui-checkbox slot="content"></ui-checkbox>
    </ui-prop>
    <ui-prop id="audioBitrateProp">
        <ui-label slot="label" value="i18n:${PACKAGE_NAME}.options.audioBitrate"></ui-label>
        <ui-num-input slot="content" step="8" min="8" max="320"></ui-num-input>
    </ui-prop>

    <ui-label class="section-title" value="i18n:${PACKAGE_NAME}.options.channels"></ui-label>
    <div id="channelsSection" class="channel-chips">${channelRowsHtml}</div>
    <ui-prop id="selectedChannelsProp">
        <ui-label slot="label" value="i18n:${PACKAGE_NAME}.options.selectedChannels"></ui-label>
 ...
 * @returns
 */
export async function update(options: IOptions, key: string) {
    if (key) {
        return;
    }
...
export async function ready(options: IOptions) {
    // @ts-ignore
...
function init() {
    ...
    panel.$.compressionType.value = panel.options.compressionType || CompressionType.Lossy;
...
function initChannels() {
    closeChannels();
    const chips: HTMLElement[] = Array.from(panel.$.channelsSection.querySelectorAll('.channel-chip'));
    channelChipBindings = chips.map(el => {
        const fieldKey = `${CHANNEL_FIELD_PREFIX}${el.dataset.channel}`;
        ...
        const handler = () => {
            ...
            panel.dispatch('update', `packages.${PACKAGE_NAME}.${fieldKey}`, next);
...
function updateSelectedChannelsSummary() {
    const selected = CHANNEL_SORTED.filter(channel => panel.options[`${CHANNEL_FIELD_PREFIX}${channel}`] === true);
    panel.$.selectedChannelsValue.textContent = selected.length ? selected.join(', ') : '—';
}

function onAppleUrlChange(event: any) {
    panel.options.appleUrl = event.target.value;
    panel.dispatch('update', `packages.${PACKAGE_NAME}.appleUrl`, panel.options.appleUrl);
    saveConfig('appleUrl', panel.options.appleUrl);
}

function onGoogleUrlChange(event: any) {
    panel.options.googleUrl = event.target.value;
    panel.dispatch('update', `packages.${PACKAGE_NAME}.googleUrl`, panel.options.googleUrl);
...
function onCompressionTypeChange(event: any) {
    panel.options.compressionType = event.target.value;
    panel.dispatch('update', `packages.${PACKAGE_NAME}.compressionType`, panel.options.compressionType);
    saveConfig('compressionType', panel.options.compressionType);
    updateCompressionQualityVisibility();
}

function onCompressionQualityChange(event: any) {
    panel.options.compressionQuality = event.target.value;
    panel.dispatch('update', `packages.${PACKAGE_NAME}.compressionQuality`, panel.options.compressionQuality);
...
function updateCompressionQualityVisibility() {
    ...
    panel.$.compressionQualityProp.style.display = panel.$.compressionType.value === CompressionType.Lossy ? 'block' : 'none';
}

function onAudioCompressionEnabledChange(event: any) {
    panel.options.audioCompressionEnabled = event.target.value;
    panel.dispatch('update', `packages.${PACKAGE_NAME}.audioCompressionEnabled`, panel.options.audioCompressionEnabled);
    saveConfig('audioCompressionEnabled', panel.options.audioCompressionEnabled);
    updateAudioBitrateVisibility();
}

function onAudioBitrateChange(event: any) {
    panel.options.audioBitrate = event.target.value;
    panel.dispatch('update', `packages.${PACKAGE_NAME}.audioBitrate`, panel.options.audioBitrate);
