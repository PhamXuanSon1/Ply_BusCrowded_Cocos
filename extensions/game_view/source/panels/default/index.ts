...
import type { CapturedCamera, CaptureResult, SelectionMarker } from '../../scene';
 ...
 */
function drawMarkers(ctx: CanvasRenderingContext2D, markers: SelectionMarker[]) {
    ctx.save();
...
async function renderComposite(cameras: CapturedCamera[], markers: SelectionMarker[], placeholder: HTMLElement) {
    if (!canvasEl) return;
...
async function capture($: any) {
    ...
    try {
        const { cameras, markers }: CaptureResult = await Editor.Message.request('scene', 'execute-scene-script', {
